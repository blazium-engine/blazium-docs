...
You have two main tools to process the player's input in Blazium:

1. The built-in input callbacks, mainly ``_unhandled_input()``. Like
   ``_process()``, it's a built-in virtual function that Blazium calls every time
...
2. The ``Input`` singleton. A singleton is a globally accessible object. Blazium
...
Blazium project. They respectively trigger when the player presses the left and
...
In summary, every script in Blazium represents a class and extends one of the
...
Blazium provides several virtual functions you can define to connect your class

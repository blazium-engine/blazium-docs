...
The Blazium IDE conveniently has a built-in profiler. It does not run every time
...
.. figure:: img/godot_profiler.png
   :align: center
   :alt: Screenshot of the Blazium profiler
...
For more info about using Blazium's built-in profiler, see
:ref:`doc_debugger_panel`.

External profilers
------------------

Although the Blazium IDE profiler is very convenient and useful, sometimes you
...
``Main::iteration()`` function. This is the master function in the Blazium source
...
outside Blazium code in ``libglapi`` and ``i965_dri`` (the graphics driver).
...
Blazium usually takes care of such low-level details for you. For example, the
...
Blazium supports a number of different languages, and it is worth bearing in mind
...
:ref:`C# <toc-learn-scripting-C#>` is popular and has first-class support in Blazium. It
...
Blazium is written in C++. Using C++ will usually result in the fastest code.
...
Each node is handled individually in the Blazium renderer. Therefore, a smaller

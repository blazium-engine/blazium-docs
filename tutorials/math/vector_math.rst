...
uses. Vectors have many applications in both 2D and 3D development and Blazium
...
notation. For example, in Blazium, the origin is the top-left corner of the
...
Blazium supports both :ref:`Vector2 <class_Vector2>` and :ref:`Vector3
...
.. note:: Vectors represent both direction and magnitude. A value representing
          ...
          :ref:`class_float` type in Blazium.
...
Because this is such a common operation, Blazium provides a dedicated
...
over when it hits the surface) and reflect it using the normal. In Blazium, there
...
using ``P - A`` and normalize it, however, Blazium has a helper method to do this
...
With Blazium, you can use the built-in :ref:`Vector3.cross() <class_Vector3_method_cross>`
method:
...
For more information on using vector math in Blazium, see the following articles:

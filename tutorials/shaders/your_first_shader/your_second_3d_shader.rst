...
From a high-level, what Blazium does is give the user a bunch of parameters that
...
shading, without writing complex shaders. Of course, Blazium also allows you to
...
fragment function in Blazium is to set up different material properties and let
Blazium handle the rest. In order to provide even more flexibility, Blazium also
...
The PBR model that Blazium uses relies on two main parameters: ``METALLIC`` and
``ROUGHNESS``.

...
.. note::
  ...
  To name a few:
    ...
    * Blazium's input system mainly centers around button inputs, XR adds triggers, axis, poses and haptics (output) into the mix.
    ...
    * Blazium's input system works with raw input data that is parsed and triggers emitting actions.
    ...
    * Blazium's input system allows changes to what inputs are bound to actions in runtime, OpenXR does not.
...
.. warning::
  ...
  It does allow a new developer to have a starting point when they want to become familiar with Blazium XR.
...
.. note::
  ...
  There are plans to extend Blazium to provide some level of control over these thresholds in the future.
...
.. warning::
  ...
  As such Blazium keeps meta data of all available devices, their inputs and outputs and which extension adds support for them.
  ...
  This does mean that in order to support new devices, you might need to update to a more recent version of Blazium.
...
.. note::
  ...
  When a controller is found, and the action map is applied to it, the XR runtime is not limited to the exact configurations you set up in Blazium's action map editor.

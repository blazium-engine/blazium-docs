...
\ **Note:** In a boolean context, an **Object** will evaluate to ``false`` if it is equal to ``null``. Otherwise, an **Object** will always evaluate to ``true``, even if it has been freed. This behavior may change in future releases. See also :ref:`@GlobalScope.is_instance_valid<class_@GlobalScope_method_is_instance_valid>`.

...
.. note:: C# support for Blazium has historically used the
...
following the instructions for building the desired Blazium binaries.
...
They can be generated by any .NET-enabled Blazium editor binary by running it with
the parameters ``--headless --generate-mono-glue`` followed by the path to an
output directory.
This path must be ``modules/mono/glue`` in the Blazium directory::

    <blazium_binary> --headless --generate-mono-glue modules/mono/glue

This command will tell Blazium to generate the C# bindings for the Blazium API at
...
Once these files are generated, you can build Blazium's managed libraries for all
the desired targets without having to repeat this process.

``<blazium_binary>`` refers to the editor binary you compiled with the .NET module
enabled. Its exact name will differ based on your system and configuration, but
should be of the form ``bin/blazium.<platform>.editor.<arch>.mono``, e.g.
``bin/blazium.linuxbsd.editor.x86_64.mono`` or
``bin/blazium.windows.editor.x86_32.mono.exe``. Be especially aware of the
**.mono** suffix! If you've previously compiled Blazium without .NET support, you
...
.. note:: The glue sources must be regenerated every time the ClassDB-registered
          ...
          Blazium will print an error at startup if there is an API mismatch
...
Unlike "classical" Blazium builds, when building with the .NET module enabled
...
proper functioning and must be distributed together with Blazium.
...
In order to use Blazium with a development version of those packages, a local
...
When building Blazium with double precision support, i.e. the ``precision=double``
...
::
    ...
    bin/blazium.windows.editor.x86_64.mono --headless --generate-mono-glue modules/mono/glue
...
The data directory is a dependency for Blazium binaries built with the .NET module
enabled. It contains important files for the correct functioning of Blazium. It
must be distributed together with the Blazium executable.

Editor
~~~~~~

The name of the data directory for the Blazium editor will always be
``GodotSharp``. This directory contains an ``Api`` subdirectory with the Blazium
...
On macOS, if the Blazium editor is distributed as a bundle, the ``GodotSharp``
...
- **module_mono_enabled**\ =yes | **no**
  ...
  - Build Blazium with the .NET module enabled.

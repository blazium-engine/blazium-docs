...
To support both architectures in a single "Universal 2" binary, run the above two commands and then use ``lipo`` to bundle them together::
    ...
    lipo -create bin/blazium.macos.editor.x86_64 bin/blazium.macos.editor.arm64 -output bin/blazium.macos.editor.universal
...
editor binary built with ``dev_build=yes``::
    ...
    cp -r misc/dist/macos_tools.app ./Blazium.app
    mkdir -p Blazium.app/Contents/MacOS
    cp bin/blazium.macos.editor.universal Blazium.app/Contents/MacOS/Blazium
    chmod +x Blazium.app/Contents/MacOS/Blazium
    codesign --force --timestamp --options=runtime --entitlements misc/dist/macos/editor.entitlements -s - Blazium.app
...
.. note::
    ...
    including the dynamic library in your ``.app`` bundle::
        ...
        mkdir -p Blazium.app/Contents/Frameworks
        cp <Vulkan SDK path>/macOS/lib/libMoltenVK.dylib Blazium.app/Contents/Frameworks/libMoltenVK.dylib
...
two commands blocks and then use ``lipo`` to bundle them together::
    ...
    lipo -create bin/blazium.macos.template_release.x86_64 bin/blazium.macos.template_release.arm64 -output bin/blazium.macos.template_release.universal
    lipo -create bin/blazium.macos.template_debug.x86_64 bin/blazium.macos.template_debug.arm64 -output bin/blazium.macos.template_debug.universal
...
``blazium_macos_release.universal`` and ``blazium_macos_debug.universal`` respectively. You can do so
with the following commands (assuming a universal build, otherwise replace the
``.universal`` extension with the one of your arch-specific binaries)::

    cp -r misc/dist/macos_template.app .
    mkdir -p macos_template.app/Contents/MacOS
    cp bin/blazium.macos.template_release.universal macos_template.app/Contents/MacOS/blazium_macos_release.universal
    cp bin/blazium.macos.template_debug.universal macos_template.app/Contents/MacOS/blazium_macos_debug.universal
